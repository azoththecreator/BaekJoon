...
그리고 1번 부터 7번까지 각 객차에 타고 있는 손님의 수가 아래와 같다고 하자.
...
int main()
{
    ...
    for (int i = 1; i <= total_carriages; ++i)
    {
        int tmp;
        cin >> tmp;
    ...
    for (int i = 1; i <= 3; ++i)
    {
        for (int j = i * small_train_carriages; j <= total_carriages; ++j)
        {
            int sum = prefix_sum[j] - prefix_sum[j - small_train_carriages];
            dp[i][j] = max(dp[i][j - 1], dp[i - 1][j - small_train_carriages] + sum);
        }
    }

    cout << dp[3][total_carriages];
